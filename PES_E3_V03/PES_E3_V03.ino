...
void drawGraph() {
  ...
  if (dataIndex > 1) {
    ...
    for (int i = 1; i < dataIndex; i++) {
      ...
      int x1 = graphX + (int)((dataPointsX[i - 1] / 10.0) * graphWidth);  // Schaal x naar scherm
      int y1 = graphY + graphHeight - (int)((dataPointsY[i - 1] / 5.0) * graphHeight);  // Schaal y naar scherm
      int x2 = graphX + (int)((dataPointsX[i] / 10.0) * graphWidth);
      int y2 = graphY + graphHeight - (int)((dataPointsY[i] / 5.0) * graphHeight);
